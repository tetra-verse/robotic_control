...
public:
     ...
     * @brief 度转弧度
     */
    static double degToRad(double degrees) { return degrees * (EIGEN_PI / 180); }

    /**
     * @brief 弧度转度
     */
    static double radToDeg(double rad) { return rad / EIGEN_PI * 180.0; }
     ...
     */
    static void transMatrixToArray(const Eigen::Matrix3d &rot,
                                   const Eigen::Vector3d &trans,
                                   std::array<double, 16> &array)
    {
        array = {{rot(0, 0), rot(0, 1), rot(0, 2), trans(0), rot(1, 0), rot(1, 1), rot(1, 2),
                  trans(1), rot(2, 0), rot(2, 1), rot(2, 2), trans(2), 0, 0, 0, 1}};
    }
    ...
    {
        array = {{R(0, 0), R(0, 1), R(0, 2), R(0, 3), R(1, 0), R(1, 1), R(1, 2), R(1, 3), R(2, 0),
                  R(2, 1), R(2, 2), R(2, 3), R(3, 0), R(3, 1), R(3, 2), R(3, 3)}};
    }
     ...
     */
    static void arrayToTransMatrix(const std::array<double, 16> &array,
                                   Eigen::Matrix3d &rot,
                                   Eigen::Vector3d &trans)
    {
        rot << array[0], array[1], array[2], array[4], array[5], array[6], array[8], array[9],
            array[10];
    ...
    {
        R << array[0], array[1], array[2], array[3], array[4], array[5], array[6], array[7],
     ...
     */
    static void postureToTransArray(const std::array<double, 6> &xyz_abc,
                                    std::array<double, 16> &transMatrix)
    {
        using namespace Eigen;
        Vector3d vec;
        Matrix3d rot;
        rot =
            (AngleAxisd(xyz_abc[5], Vector3d::UnitZ()) * AngleAxisd(xyz_abc[4], Vector3d::UnitY()) *
             AngleAxisd(xyz_abc[3], Vector3d::UnitX()))
                .toRotationMatrix();
     ...
     */
    static void transArrayToPosture(const std::array<double, 16> &transMatrix,
                                    std::array<double, 6> &xyz_abc)
    {
        ...
        xyz_abc[4] =
            atan2(-transMatrix[8], sqrt(pow(transMatrix[0], 2.0) + pow(transMatrix[4], 2.0)));
        if (fabs(xyz_abc[4]) > (EIGEN_PI / 2.0 - 1E-12)) {
     ...
     */
    static inline void toolsetCalcPos(const Toolset &tool_set,
                                      std::array<double, 16> &ref_trans,
                                      std::array<double, 16> &end_trans)
    {
     ...
     * @return 法兰相对于基坐标系坐标
     */
    static std::array<double, 6> EndInRefToFlanInBase(const std::array<double, 6> &base_in_world,
     ...
     */
    static std::array<double, 6> FlanInBaseToEndInRef(const std::array<double, 6> &base_in_world,
                                                      const Toolset &tool_set,
